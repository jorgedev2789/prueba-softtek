...
var require_middy2 = __commonJS({
  "node_modules/@aws-lambda-powertools/logger/lib/middleware/middy.js"(exports) {
    ...
    var injectLambdaContext2 = (target, options) => {
    ...
    exports.injectLambdaContext = injectLambdaContext2;
  }
});
...
// src/functions/net/application/processReport/handler.ts
var handler_exports = {};
__export(handler_exports, {
  main: () => main
});
module.exports = __toCommonJS(handler_exports);
var import_logger2 = __toESM(require_lib2());
...
var HttpFetch = class {
  constructor() {
    this.instance = axios_default.create({
      baseURL: process.env.NET_DOMAIN,
      timeout: 1e3,
...
// src/functions/net/application/processReport/processReport.ts
var ProcessReport = class {
  constructor(http2) {
    this.http = http2;
  }
  async execute(data) {
    const result = await this.http.get("pre-orders?fundState=PRE_ORDER", {
      headers: { "x-api-key": process.env.TOKEN_NET }
    });
...
// src/functions/net/application/processReport/handler.ts
var evenHandler = async (event) => {
  logger.info("event", { event });
  try {
    const recordPromises = event.Records.map(async (record) => {
      const body = JSON.parse(record.body);
      logger.info("body", body);
      const http2 = new HttpFetch();
      const processReport = new ProcessReport(http2);
      await processReport.execute(body);
    });
...
var main = core_default(evenHandler).use((0, import_logger2.injectLambdaContext)(logger, { clearState: true }));
