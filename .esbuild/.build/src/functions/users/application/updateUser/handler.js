...
// src/functions/users/application/updateUser/handler.ts
var handler_exports = {};
...
// src/functions/users/application/updateUser/updateUser.ts
var UpdateUser = class {
  ...
  async execute(userRequest) {
    try {
      ...
      if (!userRequest?.uuid) {
        throw new CustomError("Uuid es requerido", 400);
      }
      if (!userRequest?.name || !userRequest?.user_name) {
        throw new CustomError("Nombre y usuario son requeridos", 400);
      }
      if (!await this.repository.findOne(userRequest.uuid)) {
        throw new CustomError("Uuid no encontrado", 400);
      }
...
// src/functions/users/application/updateUser/handler.ts
var updateUser = async (_event) => {
  try {
    logger.info("INFO", _event.body);
    const user_data = parseBody(_event.body);
    const clientFactory = await PostgreSqlClient.getInstance().clientDB();
    const controlRepository = new UserPostgreRepository(clientFactory);
    const update = new UpdateUser(controlRepository);
    await update.execute(user_data);
...
var main = core_default(updateUser).use(
  input_output_logger_default({
    logger: async (request) => await prepareLog(request, "Actualizo un usuario "),
    awsContext: true
  })
);
